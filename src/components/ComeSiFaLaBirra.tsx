    ...
    <Section id="come-si-fa" title="COME SI FA LA BIRRA?">
      ...
      <div className="space-y-12">
        {BREWING_STEPS.map((step) => (
          <article key={step.number} className="min-w-0 break-words">
            <ContentImage
              src={step.image}
            ...
            <p className="mb-3 text-sm font-medium text-zinc-400">{step.subtitle}</p>
            <p className="leading-relaxed text-zinc-300">{step.text}</p>
          </article>
        ))}
      </div>
    </Section>
